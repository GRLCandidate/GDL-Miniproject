...
from esan.conv import GNN_node


def subgraph_pool(h_node, batched_data, pool):  # hnode: [ 4674, 16 ]
    ...
    tmp = torch.cat([torch.zeros(1, device=num_subgraphs.device, dtype=num_subgraphs.dtype),
                     torch.cumsum(num_subgraphs, dim=0)])  # [ 17 ]
    graph_offset = tmp[batched_data.batch]  # [ 4674 ]

    subgraph_idx = graph_offset + batched_data.subgraph_batch  # [ 4674 ]

    print(f'subgraph_idx: {subgraph_idx}\nnum_subgraphs: {num_subgraphs}\ngraph_offset: {graph_offset.shape}')
    sni = batched_data.subgraph_node_idx
    print('sni', sni.min(), sni.max())
    nnps = batched_data.num_nodes_per_subgraph
    print('nnps', nnps.min(), nnps.max())
    sgb = batched_data.subgraph_batch
    print('sgb', sgb.min(), sgb.max())
    print('subgraph_idx', subgraph_idx.min(), subgraph_idx.max())
    print(f'subgraph pool. num_subgraphs: {num_subgraphs} h_node: {h_node.shape} tmp: {tmp.shape} batched_data: {batched_data}')

    return pool(h_node, subgraph_idx)  # [ 523, 16 ]
...
class DSSnetwork(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        print(f'x: {x.shape} edge_index: {edge_index.shape} edge_attr is None: {edge_attr is None} batch: {batch.shape}')
        ...
        print(f'h_subgraph: {h_subgraph.shape} subgraph_idx_batch: {batched_data.subgraph_idx_batch.shape}')
        # aggregate to obtain a representation of the graph given the representations of the subgraphs
        h_graph = torch_scatter.scatter(src=h_subgraph, index=batched_data.subgraph_idx_batch, dim=0, reduce="mean")
